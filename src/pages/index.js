...
Promise.all([api.getUser(), api.getInitialCards()])
  .then(([userData, cardsArray]) => {
    ...
    cardsArray.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
...
function renderCard(item) {
  const card = new Card({
    ...
    handleRemoveCard: (item) => {
      setCardId(item.id), confirmDeletePopup.open();
    },
...
const addPlacePopup = new PopupWithForm({
  ...
  handleFormSubmit: (data) => {
    ...
    api
      .addCard(newPlace)
      .then((newCard) => {
        renderCard(newCard), addPlacePopup.close();
      })
      .catch((err) => console.log(`Error: ${err}`))
      .finally(() => {
        addPlacePopup.stopLoading();
      });
  },
});
...
addButton.addEventListener("click", () => {
  setDefaultButtonState(addPlacePopup), addPlacePopup.open();
});
...
function setDefaultValues(name, about) {
  editProfilePopup._popupElement.querySelector(".form__input_type_name").value =
    name;
  editProfilePopup._popupElement.querySelector(
...
const confirmDeletePopup = new PopupWithForm({
  ...
  handleFormSubmit: (data) => {
    api
      ...
      .catch((err) => console.log(`Error: ${err}`))
      .finally(() => {
        confirmDeletePopup.stopLoading();
      });
  },
});
confirmDeletePopup.setEventListeners();
function setCardId(id) {
  confirmDeletePopup._popupElement.querySelector(
    ".form__input_type_card-id"
  ).value = id;
}
